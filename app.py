...
# ============================================================
model   = joblib.load("C:/Users/habib/Desktop/DVF/models/best_model.pkl")
le_type = joblib.load("C:/Users/habib/Desktop/DVF/models/le_type.pkl")
le_dept = joblib.load("C:/Users/habib/Desktop/DVF/models/le_dept.pkl")
...
for dept in dept_idf:
    path = f"C:/Users/habib/Desktop/DVF/{dept}_2024.csv.gz"
...
# Stats par departement pour reference
stats_dept = df.groupby(["dept_lib","type_local"])["prix_m2"].agg(["median","mean","count"]).reset_index()
stats_dept.columns = ["dept_lib","type_local","mediane","moyenne","nb_ventes"]
...
tabs = dbc.Tabs([
    ...
    dbc.Tab(label="Simulateur ML", children=[
        ...
        dbc.Row([
            dbc.Col(dcc.Graph(id="p_sim_hist"), width=12)
        ])
    ]),
...
def kpis(_):
    ...
    return (k("Transactions 2024", f"{nb:,}".replace(",","  ")),
...
def p_prix_dept(_):
    ...
    fig = px.bar(d, x="dept_lib", y="prix_m2", color="type_local",
                 barmode="group",
...
def p_dist(_):
    ...
    fig = px.scatter(sample, x="dist_paris_km", y="prix_m2",
                     color="type_local", opacity=0.3,
                     color_discrete_map={"Appartement":"#A32D2D","Maison":"#185FA5"},
...
def simuler(type_b, dept, surface, pieces):
    # Coordonnees approx par departement
    coords = {
    ...
    resultat = dbc.Alert([
        ...
        html.P(f"Surface : {surface} m2 | {pieces} pieces | {dept_labels.get(dept,dept)}")
    ], color="success")
...
def carte_prix(type_b):
    ...
    fig = px.scatter_mapbox(d, lat="latitude", lon="longitude",
                            ...
                            hover_data={"prix_m2":":.0f","surface_reelle_bati":":.0f"},
